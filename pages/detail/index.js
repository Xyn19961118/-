...
Page({
  ...
  onLoad: function(e) {
    ...
    //请求问题描述接口
    detailM.getDetailArticle(type, id, res => {
      let msg = res.errmsg || ''
      if (res) {
        wx.hideLoading()
      }
      ...
      // console.log(res)
      //如果问题描述超过72个字，则只显示3行
      if (res.content.length > 75) {
        this.setData({
          contentIsShow: true
        })
      }
      if (type != false) {
        let commentList = this.data.answerList
        commentList.push(res)
        this.setData({
          answerList: commentList
        })
      }
      this.setData({
        content: res,
    ...
    // console.log(type,this.data.answerList)
  },
  deleteComment(e) {
    wx.showModal({
      title: '提示',
      ...
      success: result => {
        if (result.confirm) {
          detailM.deleteComment(e.detail)
        }
      }
    })
  },
  ...
  onLike(e) {
    let id = e.detail.id
    let type = this.data.type == true ? 1 : 2
    detailM.likeReply(id, type, res => {
      detailM.checkErrorCode(res)
    })
  },
  // 收藏
  onCollect(e) {
    let id = e.detail.id
    let type = this.data.type == true ? 1 : 2
    detailM.collectReply(id, type, res => {
      detailM.checkErrorCode(res)
    })
  },
  showBlock(e) {
  ...
  closeComment(e) {
    ...
    let id = e.detail.id
    let type = this.data.type == true ? 1 : 2
    detailM.commentList(id, type, res => {
      detailM.checkErrorCode(res)
      this.setData({
        allComment: res,
        replyId: e.detail.id,
